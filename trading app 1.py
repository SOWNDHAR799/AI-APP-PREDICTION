...
def get_realtime_price(symbol, mapped):
    ...
    elif mapped in {'GC=F', 'SI=F', 'CL=F'}: return None
    else: gf_sym = f"{mapped}:NASDAQ"
...
def analyze_live_candle(df):
    ...
    if lC > lO:
        ...
        desc = "Strong Bullish Breakout" if lC > pH else "Bullish Accumulation"
    elif lC < lO:
        bias, color = "DOWN 📉", "#ef4444"
        desc = "Strong Bearish Breakdown" if lC < pL else "Bearish Pressure"
    else:
        bias, color, desc = "NEUTRAL ⚖️", "#94a3b8", "Price Indecision"
...
class AIEngine:
    ...
    def train(self, symbol, prices, volumes, news_sent=0.0):
        ...
        for i in range(25, len(prices)-4):
            ...
            y4.append(1 if prices[i+4]>prices[i] else 0)
...
def page_prediction():
    ...
    st.caption("This model analyzes historical trends, technical indicators, and news sentiment for the next 3 days.")
    c1, c2, c3 = st.columns([3, 1, 1])
    with c1:
        symbol = st.text_input("Enter Stock Symbol", placeholder="RELIANCE, TATAMOTORS, GOLD...")
    with c2:
        st.write(""); st.write("")
    ...
    if run and symbol.strip():
        ...
        if live_price is not None:
            ...
            if f_data:
                st.markdown('<div class="section-head">🏛️ Fundamentals</div>', unsafe_allow_html=True)
                fc1, fc2, fc3, fc4 = st.columns(4)
                mcap_in_cr = f_data['mkt_cap'] / 10000000 if is_indian else f_data['mkt_cap']
                mcap_unit = "Cr" if is_indian else ""
...
def page_all_stocks():
    ...
    for sym in symbols[:15]:
        info = get_price_info(sym, 5)
        if info:
            rows.append({'Stock': sym, 'Price': f"{info['currency']}{info['price']:,.2f}", 'Change%': f"{info['pct']:+.2f}%"})
    if rows:
...
def page_top_movers():
    ...
    all_syms = ['RELIANCE','TCS','INFY','SBIN','TATAMOTORS','ZOMATO','AAPL','TSLA']
    data = []
    for sym in all_syms:
        info = get_price_info(sym, 5)
        if info: data.append(info)
    if data:
        gainers = sorted([d for d in data if d['pct']>0], key=lambda x:-x['pct'])
